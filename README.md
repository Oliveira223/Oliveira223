## Hello! Welcome to my GitHub
My name is **Pedro H. N. Oliveira**, I'm a **Computer Engineering student** who loves 
to understand how things work - from hardware to software.
Seeking opportunities to develop and enhance my skills in computing, with a focus on 
embedded systems and microelectronics.

**Portfolio:** [oliveira223.com.br](https://oliveira223.com.br)

### Projects
- Real State Company Dynamic [Website](https://imoveisbresolin.com.br)
- Martial Arts School Static [Website](https://blackhorses.com.br)
- Home Server: CasaOS, Jellyfin and Minecraft server via Cloudflare Tunnel
- Embedded Systems Experiments

### Currently Learning & Building
- C++: STL, pointers, data structures (university + Arduino projects)
- Hardware & Embedded: H-bridge motor control, PWM, sensors (Arduino / ESP32)
- Web: HTML · CSS · JS
- Exposure to: MIPS Assembly, Verilog/SystemVerilog (university coursework)

### Contact me
- Email: oliveira.phno@gmail.com  
- LinkedIn: [pedrooliveira223](https://www.linkedin.com/in/pedrooliveira223/)

### Languages
- 🇧🇷 Native: Portuguese
- 🇺🇸 English - Intermediate

> _"If at first you don't succeed; call it version 1.0."_
